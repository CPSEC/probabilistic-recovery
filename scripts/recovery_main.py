...
class Recovery():
    def __init__(self, dt, u_min, u_max, isolation):
        system_model = System(dt, u_min, u_max)
        self.system_model = system_model
        self.W = 0.0000001 * np.eye(system_model.n)
        mu = np.zeros((self.system_model.n))
        self.W = GaussianDistribution.from_standard(mu, self.W)
        self.rtss = RTSS(system_model.Ad, system_model.Bd, system_model.Cd, system_model.Dd, self.W, u_min, u_max, k_reconstruction=500, k_max=50 )
