...
#include "lqr_control/VehicleState.h"
...
bool LQRControllerNode::init() {
  ...
  pnh_.getParam("/vehicle_imu_topic", vehicle_imu_topic);
  ...
  pnh_.getParam("/vehicle_state_topic", vehicle_state_topic);
  ...
  lqrController_->Init();
  lqrController_->trajectory_points_ = planningPublishedTrajectory_.trajectory_points; 
  ...
  VehicleImuSub_ = nh_.subscribe(vehicle_imu_topic, 10, &LQRControllerNode::IMUCallback, this);
  // controlPub_ =
  ...
  statePub_ = 
      nh_.advertise<lqr_control::VehicleState>(vehicle_state_topic, 1000);
...
void LQRControllerNode::odomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  vehicleState_.velocity = msg->twist.twist.linear.x;
  // vehicleState_.angular_velocity = msg->twist.twist.angular.z;

  // vehicleState_.acceleration = 0.0;  // 加速度
  // cout << "linear.x=" << vehicleState_.vx << "\tangular.z=" << msg->twist.twist.angular.z << endl;
  // cout << "theta=" << vehicleState_.heading <<"  angular_v="<< vehicleState_.angular_velocity << endl;
}

void LQRControllerNode::cmdCallback(const lgsvl_msgs::VehicleControlData::ConstPtr &msg){
  vehicleState_.steering_angle = msg->target_wheel_angle;
  vehicleState_.beta = atan((vehicleState_.lr_ * tan(vehicleState_.steering_angle))/(vehicleState_.lf_ + vehicleState_.lr_));
  vehicleState_.vx = vehicleState_.velocity * cos(vehicleState_.beta);
  vehicleState_.vy = vehicleState_.velocity * sin(vehicleState_.beta);
};

void LQRControllerNode::IMUCallback(const sensor_msgs::Imu::ConstPtr &msg){
  // orientatin
  tf::Quaternion q;
  ...
  vehicleState_.acceleration = std::sqrt(msg->linear_acceleration.x * msg->linear_acceleration.x +
    msg->linear_acceleration.y * msg->linear_acceleration.y);
  
  // update lateral state
  lqrController_->UpdateState(vehicleState_);
  lqr_control::VehicleState vehicle_state;
  vehicle_state.e_cg = lqrController_->matrix_state_(0,0);
  vehicle_state.e_cg_dot = lqrController_->matrix_state_(1,0);
